...
SEAF состоит из методологии, предоставляющей рекомендации по постановке архитектурной функции (в том числе описанию различных слоев архитектуры), 
референсного инструмента (соответствующего требованиям методологии) и практик развития фреймворка (процессов исследования кейсов управления архитектурой и выявления паттернов как выверенных примеров).
...
Для структурирования метамодели SEAF используется разделение всех аспектов 
деятельности на слои (требующие сходных компетенций участников) и 
домены (вертикали, определяющие в целом направления и структуру бизнеса).

Слои выделяют функциональные области, в то время как вертикали пронизывают их 
и связывают.

Традиционно выделяются слои:
...
возможность расширения на управляемые предметные области и модификации.
...
Репозиторий содержит метамодель и соответствующий пример описания прикладной архитектуры с использованием SEAF CORE:
![Метамодель описания ПА SEAF CORE](docs/images/PrimerPA_SEAF_CORE.png)


### Метамодель SEAF CORE в применении к слою технической архитектуры
Репозиторий содержит метамодель и соответствующий пример описания технической архитектуры с использованием SEAF CORE:
![Пример описания ПА на SEAF CORE](docs/images/MM_TA.png)

### Расширение слоя технической архитектуры IAAS Reverse
Репозиторий содержит метамодель и соответствующий пример описания технической архитектуры, полученной путем автоматизированного реверса:
![Пример описания ПА на SEAF CORE](docs/images/MM_IAAS_Rev.png)

### Слой бизнес-архитектуры
Репозиторий содержит (в статусе альфа-тестирования) метамодель и соответствующий пример описания бизнес-архитектуры:
![Пример описания ПА на SEAF CORE](docs/images/MM_BA.png)
...
![ММ примера](docs/images/MM_SEAFCORE.png)
